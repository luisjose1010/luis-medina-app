...
export function Footer ({ links }: FooterProps) {
  ...
  return (
    <footer className="flex flex-col justify-between bg-brand text-seasalt p-6 min-h-44 md:px-12">
      <section className="flex justify-between gap-3 md:gap-8 max-md:flex-wrap max-lg:py-3">
        {
          links.map((link, index) => (
            <Link key={index} href={link.url} target={link.target ?? '_self'}>
              <span className="text-xs tracking-widest transition duration-300 ease-in-out hover:text-brand-800">
